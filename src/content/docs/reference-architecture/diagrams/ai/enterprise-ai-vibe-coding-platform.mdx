...
The development flow can be enabled in two ways: in a browser-based environment, employees interact through a web UI that orchestrates agents and renders live previews directly in the browser. With a local agent harness, developers use CLI-based tools integrated into their existing IDE and terminal workflow. Both paths execute AI-generated code inside a [sandboxed container](/sandbox/) with preview URLs, accelerating the feedback loop during iteration.

All LLM interactions are tracked and managed through [AI Gateway](/ai-gateway/), which provides provider routing, cost controls, prompt logging, and [DLP inspection](/cloudflare-one/policies/data-loss-prevention/). [Cost tracking](/ai-gateway/observability/costs/) attributes usage to projects, teams, departments, and individual users.

All egress from the development sandboxed container is intercepted by an [outbound handler](/containers/platform-details/outbound-traffic/), where HTTP traffic is controlled. With this outbound handler, platform administrators can allow or deny specific origin destinations, inject authorization headers or tokens, reroute traffic, apply custom policies on outbound traffic, or connect to other Cloudflare resources through [bindings](/workers/runtime-apis/bindings/). For access to on-premises or internal systems, [Workers VPC](/workers/configuration/vpc/) establishes private connectivity without exposing those systems to the Internet.
